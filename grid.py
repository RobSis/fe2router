...
class GalaxyMap():
    ...
    def _grid(self):
        ...
        for y in range(self.height * 2):
            for x in range(self.width * 2):
                self.draw.rectangle((x * factX, y * factY, (x+1) * factX, (y+1) * factY), outline="darkgreen")
        ...
        for y in range(self.height):
            for x in range(self.width):
                self.draw.rectangle((x * factX, y * factY, (x+1) * factX, (y+1) * factY), outline="lightgreen")
    ...
    def _sector(self, sector):
        ...
        if (self.grid):
            label = " %d,%d" % (sector.coordx - data.SolX, sector.coordy - data.SolY)
            self.draw.text((factX * cx, self.screensize[1] - factY * (cy + 1)), label, fill="gray")
        ...
        for star in sector.stars:
            ...
            self.draw.ellipse((absX - size, absY - size, absX + size, absY + size), fill=color) # Draw a circle
            if (self.labels):
